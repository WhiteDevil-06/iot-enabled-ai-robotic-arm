...
and drives a non-blocking asynchronous state machine to control the sorting arm.
"""
...
DEFAULT_COOLDOWN_SECONDS = 5.0  # Time for arm to sort and return
            ...
            if self.state == "READY":
                ...
                if object_detected and object_box is not None:
                    ...
                    if object_crop.size > 0:
                        ...
                        category = "rotten" if is_rotten else "fresh"
                        ...
                        if confidence >= self.args.threshold:
                            ...
                            # Fire asynchronous sorting coordinates move request
                            self.state = "SORTING"
                            last_status = f"Triggering arm move for {category}..."
                            
                            angles = ARM_SORTING_ANGLES[category]
                            move_endpoint = f"/move?base={angles['base']}&shoulder={angles['shoulder']}&elbow={angles['elbow']}&claw={angles['claw']}"
                            self.trigger_esp32_command_async(move_endpoint)
                        else:
                ...
                if not self.thread_active:
                    ...
                    last_status = "Waiting for arm motion & object clearance..."
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Real-Time Robotic Arm Fruit Sorting Classifier")
    ...
    parser.add_argument("--cooldown", type=float, default=DEFAULT_COOLDOWN_SECONDS,
                        help="Cooldown period in seconds for robotic arm sorting movements.")
